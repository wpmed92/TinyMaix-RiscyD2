...
#define TM_PRINTF_UART(...) { char buffer[100]; sprintf(buffer, __VA_ARGS__); riscy_print(buffer); }
...
static void parse_output(tm_mat_t* outs)
{
    ...
    riscy_led_pattern(maxi);

    TM_PRINTF_UART("### Predict output is: Number %d, prob %.3f\n", maxi, maxp);
...
int main() {
    ...
    uint32_t time = riscy_stop_timer();
    
    TM_PRINTF_UART("### Took %d ms\n", time);
